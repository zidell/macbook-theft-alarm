...
```zsh
git clone https://github.com/zidell/macbook-theft-alarm.git
...
입력을 마치면 `config.json`이 자동 생성되고 앱을 빌드해 감시를 시작합니다. 실행되면 검은 화면의 `CCTV 감시중` 창이 보이고, 설정한 메신저로 감시 링크가 전송됩니다. 휴대폰에서 해당 링크를 열면 영상과 `경보 울림`/`경보 끄기` 토글을 사용할 수 있으며, 화면 구석에 `live_max_seconds` 기준 "최대 OO 연동 가능" 참고 문구가 작게 표시됩니다.
